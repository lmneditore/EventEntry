...
{
//displayLMHeader();
echo "<DIV align=\"center\">You are not authorized to be here. Click on the button below. <BR><BR>";
...
$month=strtoupper($month);
	if(strlen($month)==0)
	{
	echo "</DIV ID=\"DIV1\"></td><TD><DIV ID=\"DIV1\"><P ID=\"P1\">&nbsp;$daynum&nbsp;</p>";
	}
	ELSE
	{
	echo "</DIV ID=\"DIV1\"></td><TD ><DIV ID=\"DIV1\"><P ID=\"P1\">&nbsp;$daynum&nbsp;<EM ID=\"EM3\">$month </EM></P>";
	}
}
ELSE
{
	if(strlen($month)==0)
	{
	echo "</DIV ID=\"DIV1\"></td><TD ><P ID=\"P1\">&nbsp;$daynum&nbsp;<SPAN ID=\"SPAN3\"><img src=\"$path\" width=\"50px\"></SPAN></P>";
	}
	ELSE
	{
	echo "</DIV ID=\"DIV1\"></td><TD ><DIV ID=\"DIV1\"><P ID=\"P1\">$daynum&nbsp;</p><EM ID=<\"EM3\">$month</EM><SPAN ID=\"SPAN3\"><img src=\"$path\"></SPAN></P>";
	}
}
...
function eventdata($eventtitle, $prices,$actlist, $sponsor, $seriestitle, $ages)
{
	...
	ELSE
	{
	echo "<SPAN class=\"eventtitle\">$seriestitle</span>";
	}
...
WHERE MainEvents.VenueID='$validvenue' 
...
WHERE Venues.VenueID=$validvenue";
...
$calsql="SELECT dates AS EventDate, DAYOFMONTH(dates) as DOM, MONTHNAME(dates) AS MON, YEAR(dates) AS YEAR, dates, Prices, EventStart, EventTitle, MainEVID, sponsor, SeriesTitle, PathToThumbs, AgeRestriction FROM Dates
...
<TR><TD class="dayofweekbar">Sunday</td><TD class="dayofweekbar">Monday</td><TD class="dayofweekbar">Tuesday</td><TD class="dayofweekbar">Wednesday</td><TD class="dayofweekbar">Thursday</td><TD class="dayofweekbar">Friday</td><TD class="dayofweekbar">Saturday</td></tr>
<?
$i=0;
DO
{
...
if ($MON==$thesedates["MON"])// if month hasn't changed
{
	if ($mod==0)// if the date is Sunday
	{
	...
	$ages=$thesedates["AgeRestriction"];
	$eventdate=$thesedates["EventDate"];
	$actlist="http://www.louisvillemusicnews.net/eventediting/venueschedulebox.php?thismaineventevid=".$thesedates["MainEVID"];
		echo "<TR>";
		...
		$value="Edit Event";
		$name="submit";
		$OnClick="http://www.louisvillemusicnews.net/eventediting/maineventedit.php?thiseventMainEVID=$eventid&venueid=$validvenue&submit=Edit";
$windowlocation="parent.location";
$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
	...
	$value="Add Event";
	$name="submit";
	$OnClick="http://www.louisvillemusicnews.net/eventediting/maineventedit.php?venueid=$validvenue&submit=New&thisdate=$eventdate";
$windowlocation="parent.location";
$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
		echo "</B></DIV>";
				}
