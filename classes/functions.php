...
function verifycalendaruser($user_id)
{
    ...
    if (isset($_SESSION["valid_user"]))
    {
		IF (!isset($_SESSION["user_id"]))
			...
			$check=@mysql_query($sqlck, $connection4) or die("couldn't execute validate user query ");
			...
			//check for buttons to display
				...
				$$typename=$checkapprov["id_value"];
					...
					$name="";
				$OnClick="http://www.louisvillemusic.com/eventediting/editthisvenueevents.php?validvenue=$validvenue";
					$windowlocation="";
					lmnbutton($value, $name,$OnClick,$windowlocation);
						echo "<BR><BR>";
					}
					...
					$value="Edit Act";
					$name="";
					$OnClick="http://www.louisvillemusic.com/eventediting/Actinfo.php=$validact";
					$windowlocation="window.location";
					lmnbutton($value, $name,$OnClick,$windowlocation);
					echo "<BR><br>";
					}
					...
					$name="";
					$OnClick="http://www.louisvillemusic.com/teachers/teacherinfo.php?submit=Show_Teacher&teacherid=$teacherid";
					$windowlocation="window.location";
					lmnbutton($value, $name,$OnClick,$windowlocation);
					echo "<BR><BR>";
					}
...
/*			IF ($validact)
    ...
    <Table width="360" align="center" class="centertable"><tr><td class="loginhdr">You are not authorized to be here. Please return to the Main Page or log in.<? echo $valid_user;?></td></tr>
    <tr><td align="center"><BR>
<?	
$value="Log in";
$name="submit";
$OnClick="window.location='http://www.louisvillemusic.com/members/lmlogin.php'"; 
lmnbutton($value, $name,$OnClick,$windowlocation);
?></td></tr>
    <tr><td align="center"><BR>
	<?
	$value="Main";
	$name="submit";
	$OnClick="http://www.louisvillemusic.com/lmn/lmhdr.php";
	lmnbutton($value, $name,$OnClick,$windowlocation);
?></td></tr></table>
     <? 
     }
...
echo "<tr><td class=\"IssueHdr\" width=\"580\">$loginheader</font></td></tr>\n";
	...
	function displayLMHeader( $title = "Louisville Music.com Main Menu" )
	{
		...
		echo "<table  width=\"600\" align=\"center\" class=\"centertable\">";
echo "<tr><td align=\"center\" bgcolor=\"white\" colspan=\"5\" valign=\"top\">"; 

   echo ("<img src=\"http://www.louisvillemusic.com/lmn/newlmnwebflagw.jpg\" width=\"600\" align=\"center\"></td></tr><tr><td bgcolor=\"#0c4a7d\" colspan=\"5\">");

      include ("http://www.louisvillemusic.com/lmn/menuwhite.php");
	 
      ?>
		</td></tr>
		<tr><td align="center" width="600" style=" margin: 0px; 0px; 0px; 50px; cellpadding: 2; background-color: #1F71AD; color:#f5e3a7; text-align: center; line-height: 11.5pt; font-size: 10.5pt; font-weight: bold; font-family: arial;" colspan="5" valign="top" background="#0c4a7d"> 
     <?PHP
      echo date("l, F j, Y");
      ?>
      </td></tr>
...
$check=@mysql_query($sql, $connection4) or die("Couldn't check on this change.");
...
if ($changedata["mainevid"]==NULL)
{
        $sqlpost="INSERT INTO eventchange VALUES ('','$thiseventMainEVID','$thisschedid', '$changedate','0000-00-00','$user_id','$typechange','$oldschedid')";
         $result=@mysql_query($sqlpost, $connection4) or die("couldn't execute post change for".$schedid."query.");
return;
}
ELSEIF ($sent2media=="0000-00-00")// If media info has not been sent, update record to add schedule id.
{
         ...
         $result=@mysql_query($sqlpostchange, $connection4) or die ("Unable to revise the eventchange record for ".$thiseventMainEVID."and ".$thisschedid);
return;
}
...
$rec=@mysql_query($sqlmaineventup, $connection4) or die("couldn't execute mainevents delete query for MainEVID=".$thiseventMainEVID.".s");
$result=@mysql_fetch_array($rec);
	...
	$results=@mysql_query($sqlmaineventinsert, $connection4) or die("couldn't execute insert MainEvent_Old query ");
	}
...
ORDER BY bnewsid";
$selectresult=@mysql_query($sql, $connection4) OR DIE ("Unable to select breaking news table.");
...
<!-- Copyright 2005 By <? echo $company; ?>. All rights reserved. Portions of text may be used in accordance with the doctrine of "Fair Use," provided proper credits and referencing to <? echo $company; ?> at <? echo $url;?> . Reuse of photographs on this site is strictly prohibited. Any unauthorized use of this document and the content herein is strictly forbidden.-->
<?
if (!$meta)
{
...
<META name=\"Keywords\" content=\"music,news, Louisville,Kentucky,Southern, Indiana, live,blues,rock, country, bluegrass, Christian, Celtic, hardcore, musicians, jazz, mountain, original, reviews, features, events, songwriters\">
...
{
		$OnClick="http://www.louisvillemusic.com/members/lmlogout.php";
		$windowlocation="parent.location";
		lmnbutton("Log Out","Submit", $OnClick,$windowlocation);
		$redo="http://www.louisvillemusic.com/members/membersonly.php";
		?>&nbsp;&nbsp;<?
		$OnClick=$redo;
		$windowlocation="parent.location";
...
function displayHeader()
	...
	$title = "Louisville Music.com Main Menu ";
		...
		echo "<link rel=\"stylesheet\" href=\"http://www.louisvillemusic.com/stylesheets/LMNStyleswhite.css\" type=\"text/css\">\n"; 
		?>
...
window.onload = changebgcolor
		...
		echo ("<img src=\"http://www.louisvillemusic.com/lmn/newlmnwebflagw.jpg\" width=\"583\" align=\"center\"></DIV><DIV class=\lmhdr\">");
...
/*	function loginbutton()
	...
	$value="Log In";
	$name="submit";
	$OnClick="http://www.louisvillemusic.com/members/lmlogin.php";
	lmnbutton($value, $name,$OnClick,$windowlocation);
	}
*/
...
global $thisimage;
	...
	{
	$pathtoimage=eregi("louisvillemusicnews.com", "louisvillemusic.com",$pathtoimage);
