		...
		IF (!isset($_SESSION["user_id"]))
			...
			//check for buttons to display
				if ($user_id==24)
				{
					...
					lmnbuttonA($value, $name,$OnClick,$windowlocation);?>
					<BR><BR><?
					}
				...
				$OnClick="http://www.louisvillemusicnews.net/eventediting/editthisvenueevents.php?validvenue=$validvenue";
					...
					$value="Edit Act";
					$name="";
					$OnClick="http://www.louisvillemusicnews.net/eventediting/Actinfo.php=$validact";
					$windowlocation="window.location";
					lmnbuttonA($value, $name,$OnClick,$windowlocation);
					echo "<BR><br>";
					}
...
/*			IF ($validact)
    ...
    <div class="loginhdr">You are not authorized to be here. Please return to the Main Page or log in.<? echo $valid_user;?></div>
...
function displayloginheader($loginheader)
	{
		echo "<DIV class=\"mainloginhdr\" >$loginheader</DIV>\n";
		
echo "<DIV class=\"lmhdr\">"; 
   echo ("<img src=\"http://www.louisvillemusicnews.net/lmn/newlmnwebflagw.jpg\" width=\"600\" align=\"center\"></DIV><DIV class=\"menubox\">");
...
{
echo "<DIV class=\"centerdiv\" colspan=\"".$colspan."\">You are not authorized to access this page. <BR>Please contact the site adminstrator for assistance.</DIV>";
echo "<DIV class=\"centerdiv\"><button OnClick=\"parent.location='http://www.louisvillemusicnews.net'\" style=\"background-color: \"grey\"><font color=\"blue\">Click Here</button></div>";
...
function lmnbuttonA($value, $name,$OnClick,$windowlocation)
{
$len=strlen($value);
echo "<style>";
echo ".lmnbutton";
...
function documentheaderwstyles($company, $stylesheet1,$stylesheet2,$url,$meta)
{
	if (!$company)
	{
	...
	if(!$stylesheet1)
	{
	$stylesheet1="http://www.louisvillemusicnews.net/stylesheets/LMNStyleswhite.css";
	}
	if (!$url)
	{
...
<HTML>
<HEAD>
<LINK REL="stylesheet" HREF="<? echo $stylesheet1;?> " TYPE="text/css">
<LINK REL="stylesheet" HREF="<? echo $stylesheet2;?> " TYPE="text/css">
<TITLE>
...
function documentheaders($company, $stylesheet,$url,$meta)
{
	...
	if(!$stylesheet)
	{
	$stylesheet="http://www.louisvillemusicnews.net/stylesheets/LMNStyleswhite.css";
	}
	if (!$url)
	{
	$url="www.louisvillemusicnews.net";
