...
$month=strtoupper($month);
	...
	ELSE
	{
	echo "&nbsp;$daynum&nbsp;<EM ID=\"EM3\">$month </EM></P></DIV>";
	}
}
ELSE
{
	if(strlen($month)==0)
	{
	echo "&nbsp;$daynum&nbsp;<SPAN ID=\"SPAN3\"><img src=\"$path\" width=\"50px\"></SPAN></P></DIV>";
	}
	ELSE
	{
	echo "$daynum&nbsp;</p><EM ID=<\"EM3\">$month</EM><SPAN ID=\"SPAN3\"><img src=\"$path\"></SPAN></P></DIV>";
	}
}
...
// submit a date
$somedate=strtotime($thisstartdate);//create a TIMESTAMP
...
$calsql="SELECT DAYOFMONTH(dates) as DOM, MONTHNAME(dates) AS MON, YEAR(dates) AS YEAR, dates, Prices, EventStart, EventTitle, MainEVID, sponsor, SeriesTitle, PathToThumbs, AgeRestriction,  moonphaselist.PhaseImgPath AS IMG FROM Dates
LEFT JOIN VenueEvents
ON Dates.dates=VenueEvents.EventDate
LEFT JOIN MoonPhases ON
Dates.dates=MoonPhases.Date
LEFT JOIN moonphaselist ON
MoonPhases.Phase=moonphaselist.moonphaseid
...
if ($MON==$thesedates["MON"])//month hasn't changed
{
	if ($mod==0)//indicates Sunday
	{
	$daynum=$thesedates["DOM"];
	$eventtitle=$thesedates["EventTitle"];
	$prices=$thesedates["Prices"];
	$seriestitle=$thesedates["SeriesTitle"];
	$month="";// month is empty string
	$eventid=$thesedates["MainEVID"];
	$ages=$thesedates["AgeRestriction"];
	$path=$thesedates["PathToThumbs"];
	$ages=$thesedates["AgeRestriction"];
	$thisdate=$thesedates["dates"];
	$actlist="http://www.louisvillemusicnews.net/test/venueschedulebox.php?thismaineventevid=".$thesedates["MainEVID"];
	$moonicon=$thesedates["IMG"];
		echo "<TR><a href=\"http://www.louisvillemusicnews.net/eventediting/maineventedit.php?venueid=$validvenue&thisdate=$thisdate&thiseventMainEVID=".$thesedates["MainEVID"]."\" text=\"Click to create a new event on this date\"><TD valign=\"top\">";//start a new row and create table data box
		if (!$moonicon)
		{
		echo "<DIV ID=\"DIV1\"><P ID=\"P1\">";// default
		}
		ELSE
		{
		echo "<DIV ID=\"DIV1\"><P ID=\"P1\"><img src=\"".$moonicon."\" align=\"left\" width=\"15\" >";
		}
		datebox ($daynum, $month, $path);
		if ($eventid)
		{
		$thiseventMainEvid=$eventid;
	...
	$ages=$thesedates["AgeRestriction"];
	$thisdate=$thesedates["dates"];
	echo "<a href=\"http://www.louisvillemusicnews.net/eventediting/maineventedit.php?venueid=$validvenue&thisdate=$thisdate&thiseventMainEVID=".$thesedates["MainEVID"]."\" onMouseOver=\"window.status='Click to create a new event on this date'; return true;\" onMouseOver=\"window.status=''; Return true;\"><TD valign=\"top\">";
...
ELSE//Name of Month has changed
{
if ($mod==0)
	{
	$daynum=$thesedates["DOM"];
...
//break columns here start a new row
		echo "<TR><a href=\"http://www.louisvillemusicnews.net/eventediting/maineventedit.php?venueid=$validvenue&thisdate=$thisdate&thiseventMainEVID=".$thesedates["MainEVID"]."\" text=\"Click to create a new event on this date\"><TD valign=\"top\">";
		if (!$moonicon)
		{
		echo "<DIV ID=\"DIV1\"><P ID=\"P1\">";
		}
	...
	$thisdate=$thesedates["dates"];
	$moonicon=$thesedates["IMG"];
	echo "<a href=\"http://www.louisvillemusicnews.net/eventediting/maineventedit.php?venueid=$validvenue&thisdate=$thisdate&thiseventMainEVID=".$thesedates["MainEVID"]."\" text=\"Click to create a new event on this date\"><TD valign=\"top\">";
		if (!$moonicon)
		{
		echo "<DIV ID=\"DIV1\"><P ID=\"P1\">";
		}
