...
$cnxn-> lm_connection($dbname);
	$linkssql="SELECT linkxacts.linkid AS LinkID, lmn_links.LinkTitle as act_link_title, lmn_links.Link_URL as act_link_url, lmn_links.linkcategories as act_link_categories, lmn_links.Description as act_link_descrip  FROM lmn_links LEFT JOIN linkxacts ON lmn_links.LinkID=linkxacts.linkid WHERE linkxacts.actid='".$ActID."'";
//	echo $linkssql;
...
/*		$numtypes=count($types);//count the array of kinds of prefixes
		...
		$val=strip_tags($val);
			...
			$thislen=strlen($val)-3; // find the current key length without the prefix
				if (substr($val,3,$thislen)=="LinkID")// check to see if the current key is primary.
				{
					...
					ELSE // The primary key is not empty; continue updatestring
					{
						...
						if (!$$thislink) // Check to see if the url value is empty
						{
						$$updatestring="Delete FROM act_links WHERE LinkID='".$$val."';\n";//
						}
				...
				if (substr($val,3,$thislen)=="LinkID")// check to see if the current key is primary.
				{
					...
					{ // if primary key is empty, scheck to see if the act_link_url is empty
						...
						$newrec="INSERT INTO act_links VALUES ('','$ActID','','','','','','','','','','','')";
							...
							$lastid=mysql_insert_id();
							$whereupdate=$whichprefix."whereupdate";
							$$whereupdate=" WHERE LinkID='$lastid'";
						}
					}
					ELSE // The primary key is not empty; continue updatestring
					{
...
$thisdescripcolor=$pre."act_link_descripcolor";
$thisdescriplen=$pre."act_link_descriplen";
?><table id="actlinks"><input type="hidden" name="<?php echo $thislinkurl;?>" value="<?php echo $$thislinkurl;?>">
...
<input type="text" name="<?php echo $thislinkurl1;?>" value="<?php echo $$thislinkurl;?>" size="<?PHP echo $$thislinklen;?>" style="color:<?PHP echo $$thislinkurlcolor;?>;" onBlur="IsURL(this);"></td><td class="td2">
			...
			<SELECT name="<?php echo $thislinktype1;?>" style="size: 1; width: 8px; color:<?PHP echo $$thislinktypecolor;?>;">
<?PHP
			$dbname="louisvil_louisvillemusic2";
//echo $dbname;
		$cnxn-> lm_connection($dbname);
			$fieldname="CategoryName";
			$keyfieldname="linkcategoryid";
			$outtable="linkcategories";
			if ($$thislinktype=="")
			{
			$selvalue=$value;
			}
...
//			$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
			echo "<a href=\"http://www.louisvillemusicnews.net/eventediting/actlink.php?LinkID=".$$thisprimarykey."&submit=Edit\">";
			$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
			echo "</a>";
			}
			ELSE
