...
<LINK REL stylesheet="http://www.louisvillemusicnews.net/main/venueedit.css" type="text/css">
</head>

<body bgcolor="#0F3925"><?PHP
...
?>
    ...
    <TH Colspan=2 class="searchhdr">Main Event Update Form for <B><?PHP echo $venuename;?> &  Event Number&nbsp;<?PHP echo $thiseventMainEVID; ?> on <?PHP echo $thisdate;?> & <?PHP echo $thisdoor;?></b> </th>
...
</td></tr> <!-- end of age and status lookup sectiopn -->
    <TR><TD align="center" colspan="2">
	
<?PHP echo "<hr width=\"240\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";
 inputbutton("submit","submit", "Accept",$OnClick);
...
?></td></tr>
     <tr><td align="right"> Age Restrictions: </td><td>
      ...
      <TR><TD align="center" colspan="2">
	  &nbsp;<?PHP inputbutton("submit","submit","Final Delete",$onClick);
...
?>
	  &nbsp;
	<?PHP  inputbutton("submit","submit","Add New Event",$OnClick);?>
...
<!-- <INPUT TYPE="Button" VALUE="Edit Another Event" onClick="parent.location = 'http://www.louisvillemusicnews.net/eventediting/editthisvenueevents.php?'">--></form></td></tr>
...
<?PHP 
	  ...
	  <?PHP echo "<hr width=\"240\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";
	  inputbutton("submit", "submit", "Update",$OnClick);?>
	 &nbsp;
	<?PHP 
	$value="Delete";
	$name="";
...
<?PHP	 //-- START OF BUTTON SECTION --------------------------------------------------->
	 echo "<hr width=\"240\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";
		$value="Update";
	inputbutton("submit","submit",$value,$OnClick);?>
	 &nbsp;
	<?PHP 
		$value="Delete";
		$name="";
...
echo "<hr width=\"360\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";
	...
	$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
		...
		{
		echo "<hr width=\"360\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";
		$value="Repeat This Event";
		$OnClick="http://www.louisvillemusicnews.net/eventediting/maineventedit_a.php?venueid=$validvenue&submit=Add_Plus&plus=1";
		$windowlocation="parent.location";
		echo"&nbsp;";
	$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
		}
...
?> </td></tr></table><?PHP
	...
	$result=@mysql_query($sql) or die("couldn't execute maxevid query.");
	$evid=@mysql_fetch_array($result);
	$newevid=$evid["MAXIMUM"];
	$newevid=$newevid+1;
	$sql="SELECT * FROM MainEvents WHERE MainEVID=$thiseventMainEVID";
	$resultME = @mysql_query($sql) or die("couldn't execute initial mainevents query because ".$validvenue." is not set and ".$thiseventMainEVID." is not set");
	$fieldnames=@mysql_fetch_array($resultME);
	$numfields=@mysql_num_fields($resultME);
	$insertrec="INSERT INTO MainEvents Values (";
			for ($i=0; $i<$numfields; $i++) 
			{
			$thisfieldname  = mysql_field_name($resultME, $i);
			$$thisfieldname=$fieldnames[$thisfieldname];
				if ($thisfieldname=="MainEVID")
				{	$insertrec .="'".$newevid."',";
				}
				ELSEIF ($thisfieldname=="EventDate")
				{	$thisdate=$$thisfieldname;
					...
					$thisdate=$olddate+(60*60*24*$plus);
					$thisdate=date("Y-m-d",$thisdate);
					$insertrec .="'".$thisdate."',";
				}
				ELSEIF ($i==($numfields-1))
				{
				$insertrec .="'".$$thisfieldname."')";
				}
				ELSE
				{
				$insertrec .="'".$$thisfieldname."',";
		...
		$sql="SELECT * FROM MainEvents WHERE (EventDate=$thisdate AND EventStart='$EventStart' AND VenueID=$validvenue)";
		$datecheck=@mysql_query($sql,$connection4) OR DIE ("Unable to check this date");
		$numrecs=mysql_num_rows($datecheck);
			If ($numrecs>0)
			{
			?>
			<SCRIPT Language="JavaScript" type ="Text/Javascript">
			{
			alert("There is already an event scheduled at this venue for that date and time. Please confirm all information before attempting to add a new record for this date.")
			}
			</script>
			<?PHP
			$thisdate=$EventDate;
			...
			ELSE
			{
			$newrecord=@mysql_query($insertrec,$connection4) OR DIE ("Unable to insert this new record.");
			}
			$sqlsched="SELECT * FROM ScheduleOfEvents WHERE MainEVID=$thiseventMainEVID";
			$schedresult=@mysql_query($sqlsched,$connection4) OR DIE ("Unable to select these ScheduleOfEvents items.");
			$numrecs=@mysql_num_rows($schedresult);
			
				for ($i=0; $i<$numrecs; $i++) 
				{
				$sql="SELECT MAX(ScheduleID) as MAXIMUM FROM ScheduleOfEvents WHERE 1";
				$result=@mysql_query($sql) or die("couldn't execute ScheduleID query.");
				$evid=@mysql_fetch_array($result);
				$newschedid=$evid["MAXIMUM"];
				$newschedid=$newschedid+1;
				$insertsched="INSERT into ScheduleOfEvents Values (";
				$thisfieldname  = mysql_field_name($schedresult, $i);
				$$thisfieldname=$fieldnames[$thisfieldname];
					if ($thisfieldname=="ScheduleID")
					{	$insertsched .="'".$newschedid."',";
					}
					ELSEIF ($thisfieldname=="MainEVID")
					{
					$insertsched .="'".$newevid."',";
					}
					ELSEIF ($i==($numfields-1))
				...
				$schedinsert=@mysql_query($insertrec,$connection4) OR DIE ("Cannot insert this new schedule item");
				}
echo $schedinsert;
		}

$thiseventMainEVID=$newevid;
$thistitle=$EventTitle;
$thisseries=$SeriesID;
$validvenue=$VenueID;
$thisdoors=$DoorsOpen;
$thiseventstarttime=$EventStart;
$thiseventendtime=$EventEnd;
$thisprice=$Prices;
$thisdescription=$DescriptionOfEvent;
$thisadvertiser=$Advertiser;
$thisagecode=$AgeLimits;
$onsale=$onsaledate;
$input_site=1;
$thisstatus=$status;
$thissponsor=$sponsor;
...
?>
    ...
    <TR><td colspan="2" align="center" style="color:red;">(<I><B>This Record Has Been Successfully Added.</I></b>)</td></tr>
    <form action="maineventedit.php" method="GET">
    <input type="hidden" name="validvenue" value="<?PHP echo $validvenue;?>">
    <input type="hidden" name="thiseventMainEVID" value="<?PHP echo $thiseventMainEVID;?>">
    <tr><td align="right">Event Date:</td><td> 
