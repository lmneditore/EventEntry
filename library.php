...
function displayLMHeader()
	{
		...
		$display .= "<link rel=\"stylesheet\" href=\"ubunt/classes/currentcss.css\" type=\"text/css\">\n";
		$displaly .="<LINK REL stylesheet=\"http://www.louisvillemusicnews.net/lmn/stylesheets/venueedit.css\" type=\"text/css\">";
...
$thisrow = @mysql_fetch_array($result);
	$display .= "<select name=\"AgeCodeID\" style=\"color:".$textcolorAgeCodeID.";\">";
	    DO
	    {
...
function WriteDateSelect($BeginYear, $EndYear,$IsPosted = true,$prefix,$thisdate, $textcoloryear, $textcolormonth,$textcolorday)
{
  ...
  $display .="<select  name=\"".$pyearname."\" style=\"color:".$textcoloryear.";\">";
  for ($i = $BeginYear; $i <= $EndYear; $i++)
  {
	  $selected=(($i==$pyear) ? "SELECTED" : "");
	  $display .="<option name=\"".$pyearname."\"  ".$selected." >".$i."</option>";
  }
  $display .= "</select><select  name=\"". $pmonthname."\" style=\"color:".$textcolormonth.";\">";
  ...
  $display .= "</select><select name=\"".$pdayname."\" style=\"color:".$textcolorday.";\">";
  for ($i = 1; $i <= 31; $i++)
  {
	    $selected=(($i==$pday) ? "SELECTED" : "");
    $display .= "<option ".$selected." value=\"".$i."\">".$i."</option>";
  }
 $display .="</select>";
 echo $display;
}
