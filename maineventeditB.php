...
if ($_action=="EditThisEvent" || $_action=="Return To This Event")
{
//$_action="Edit";
...
//combine On Sale date parts
   ...
   $postkeys=array("onsaledate1","EventStart1","EventEnd1","DoorsOpen1","status1","EventTitle1","sponsor1","Prices1","DescriptionOfEvent1","AgeCodeID1","eventmonth1","eventday1","eventyear1,AgeLimits1");
      //AgeLimits1,validvenue,MainEVID,VenueID"

foreach($_GET as $key=>$val)
{
	if($key==("_action") || $key==("MainEVID"))
		{
		}
	ELSE
	{
		if((substr($key,-1)=="1"))
		{
			$oldkey=((substr($key,-1)=="1") ? substr($key,0,strlen($key)-1): $key);
			if(isset($key)==TRUE)
			{
				$colorkey="textcolor".$oldkey;
				$keyname=$key;
				$$colorkey=(($val==$$oldkey) ? $defaulttext: $changedtext);
				$oldkey=(($val==$$oldkey)  ? $$oldkey: $val );
				echo $oldkey."=".$$oldkey."<BR />";
				echo $colorkey."=".$$colorkey."<BR />";
			}
		}  
	}	
}  
...
?>
<div class="searchhdr" id="MainEventUpdate">Main Event Update Form for <B><?PHP echo $VenueName;?>&  Event Number&nbsp;<?PHP echo $_session["MainEVID"]; ?></b></div>
...
<?PHP 
	  ...
	  $selector->WriteDateSelect($BeginYear, $EndYear,$IsPosted = true,$prefix,$EventDate, $textcoloreventyear, $textcoloreventmonth,$textcoloreventday);
	   echo $selector->date;
echo "</div><div class=\"SingleListLeft\">Currently:</div> <div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorEventDate.";\" value=\"".$EventDate."\" READONLY size=\"10\" name=\"EventDate1\"></div>";
echo "<div class=\"SingleListLeft\">Event Title: </div><div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorEventTitle."\" name=\"EventTitle1\" value=\"".$EventTitle."\" size=\"50\"></div>";
echo "<div class=\"SingleListLeft\">Sponsor Credit:</div><div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorsponsor.";\" name=\"sponsor1\" value=\"".$sponsor."\" size=\"50\"></div>";
echo "<div class=\"SingleListLeft\">Event Start:</div><div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorEventStart."\" name=\"EventStart1\" value=\"".$EventStart."\" size=\"11\">"; 
echo "</div><div class=\"SingleListLeft\">Event End: </div><div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorEventEnd."\" name=\"EventEnd1\" value=\"".$EventEnd."\" size=\"11\">";
echo "</div><div class=\"SingleListLeft\">Doors Open:</div><div class=\"SingleListRight\"><input type=\"text\" style=\"color:".$textcolorDoorsOpen."\" name=\"DoorOpen1\" value=\"".$DoorsOpen."\" size=\"11\">"; 
echo "</div><div class=\"SingleListLeft\">Prices:</div><div class=\"SingleListRight\"> <input type=\"text\" style=\"color:".$textcolorPrices."\" name=\"Prices1\" value=\"".$Prices."\" size=\"20\"></div>"; ?>
...
<?PHP 	  
	...
	$selector->WriteDateSelect($BeginYear, $EndYear,$IsPosted = true,$prefix,$onsaledate, $textcoloronsaleyear, $textcoloronsalemonth,$textcoloronsaleday);
	 echo $selector->date;
	echo "&nbsp;&nbsp;Currently: <input type=\"text\" style=\"color:".$textcoloronsaledate.";\" value=\"".$onsaledate."\" READONLY size=\"10\" name=\"onsaledate1\"></div>";
echo "<div class=\"SingleListLeft\">Description:</div><div class=\"SingleListRight\"> <textarea cols=\"20\" rows=\"3\" style=\"color:".$textcolorDescriptionOfEvent.";\" name=\"DescriptionOfEvent1\">".$DescriptionOfEvent."</textarea></div>";
...
echo "</div><div class=\"containerrow\" id=\"buttonstart\"  style=\"text-align:center;margin:auto;\">";
...
echo "</div><div class=\"SingleListLeft\">Display Status:&nbsp;</div><div class=\"SingleListRight\"> ";
$library->statusselectbox($status,$textcolorstatus);
echo "</div><div class=\"containerrow\" id=\"buttonstart\" style=\"text-align:center;margin:auto;\"><hr width=\"240\" size=\"1 px\" background-color=\"#f5e3a1\" text-align=\"center\">";
 	$value="Update Event";
	$name="_action";
	$type="submit";
	$OnClick=$library->basepath."index.php";
	$windowlocation="window.location";
	$buttons-> forminputbutton($type,$name,$value,$onclick);
	echo $buttons->button;
	echo "</form></div>";
...
echo "<div class=\"containerrow\" style=\"text-align:center;margin:auto;\"><hr width=\"300\" size=\"1 px\" background-color=\"#f5e3a1\" align=\"center\">";  
?>
<FORM action="http://www.louisvillemusicnews.net/lmn/eventediting/index.php">
<?PHP
echo "<input type=\"hidden\" name=\"MainEVID\" value=\"".$_session["MainEVID"]."\";>";
	//$value="Update Event";
//	$name="_action";
	//$OnClick="index.php";
	//$windowlocation="window.location";
...
//	echo $buttons->button;
	...
	$name="_action";
	$OnClick=$path."index.php?validvenue=".$VenueID."";
	$windowlocation="parent.location";
	$buttons->lmnbutton($value, $name,$OnClick,$windowlocation);
	echo $buttons->button;
